...
bottomUp :: Functor (p a1) => (p a1 (Rec q a b) -> q a (Rec q a b)) -> Rec p a1 b -> Rec q a b
bottomUp f (Place b) = Place b
bottomUp f (Roll par) = Roll . f $ fmap (bottomUp f) par

topDown :: Functor (q a) => (p a1 (Rec p a1 b) -> q a (Rec p a1 b)) -> Rec p a1 b -> Rec q a b
topDown f (Place b) = Place b
topDown f (Roll par) = Roll . fmap (topDown f) $ f par

{--
topDown f (Place b) = Place b
topDown f (Roll par) = Roll . fmap (topDown f) <<< out <<< f 
...
-- universal to all PHOAS languages
var = Place
